...
public class GameBoardHard extends JPanel implements ActionListener {
	...
	// need the dimensions for the height n width of the ghost1 image
	public int ghost5width = 20;
	public int ghost5height = 20;
	...
	// These two arrays store the x, y coordinates of all possible positions on
	// the board for ghost bot 5
	public int ghostx5[] = new int[Maximum_pixels_on_screen];
	public int ghosty5[] = new int[Maximum_pixels_on_screen];
	...
	// for the game
	public GameBoardHard(GameFrame frame, // game window created in the GameStart
										// class
...
//				// -----------------------------------------------------------------------------
	...
	private void loadGhost5(String Ghost5Image) {

		// added second pacman player
		try {
			ghost5 = ImageIO.read(getClass().getResourceAsStream(
					Ghost5Image));
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	// this method will move the pacman players in desired direction, using its
		...
		// but majority of the code is ours - this applies to all GameBoard Classes
	public void pacmove() {
	...
	public void Ghostmove() {
		...
		// 5th ghost bot movement
				if (ghostmoveup) {
					ghosty5[0] = ghosty5[0] - 20; // is the size of each ghost - so will move a
										// whole tile
				}

				// This line moves pacman down.
				if (ghostmovedown) {
					ghosty5[0] = ghosty5[0] + 20;
				}

				// This line moves pacman to the right.
				if (ghostmoveright) {
					ghostx5[0] = ghostx5[0] + 20;
				}
	...
	public void checkGhostCollision() {
		...
		// collision for ghost 4
				...
				// collision for ghost 5
				if ((playerx1[0] == ghostx5[0]) &&  (playery1[0] == ghosty5[0])) {
				...
				if (( playerx2[0] == ghostx5[0]) &&  ( playery2[0] == ghosty5[0])) {
		...
		if (playing) {
			...
			for (int i = 0; i < pac1; i++) {
				if (i < 1) {
					...
					graphics.drawImage(ghost5, ghostx5[0], ghosty5[0], null);
